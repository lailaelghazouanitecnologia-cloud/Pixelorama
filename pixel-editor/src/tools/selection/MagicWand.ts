 ...
 * Selects contiguous areas of similar color
 * Based on Pixelorama's MagicWand.gd
 */
...
export class MagicWandTool extends BaseSelectionTool {
  ...
  setConfig(config: Partial<MagicWandConfig>): void {
  ...
  private selectArea(pos: Point): void {
    if (!this.ctx || !this.canvas) return
    ...
    if (this.contiguous) {
      ...
      this.floodSelect(data, mask, width, height, pos, targetColor)
    } else {
      // Select all pixels with matching color
      this.globalSelect(data, mask, width, height, targetColor)
    }
  ...
  private floodSelect(
    ...
    while (stack.length > 0) {
      ...
      if (!this.colorsMatch(targetColor, color)) continue
...
// Tool definition
export const MagicWandDefinition = defineToolWithFactory(
  'magicWand',
  'Magic Wand',
  'wand', // Icon name
  ...
  {
    shortcut: 'W',
    hint: 'Magic Wand (W). Select similar colors. Shift: add, Alt: subtract'
  }
)

// Register the tool
ToolRegistry.register(MagicWandDefinition)
