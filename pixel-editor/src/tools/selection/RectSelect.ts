 ...
 * Based on Pixelorama's RectSelect.gd
 */
...
export class RectSelectTool extends BaseSelectionTool {
  private startPos: Point | null = null
  private currentRect: Rect = { x: 0, y: 0, width: 0, height: 0 }
  private isSquare = false // Shift key
  ...
  drawMove(pos: Point, event?: PointerEvent): void {
    ...
    this.isSquare = event?.shiftKey ?? false
...
// Tool definition
export const RectSelectDefinition = defineToolWithFactory(
  'rectSelect',
  'Rectangle Select',
  'square-dashed', // Icon name
  'selection' as ToolCategory,
  () => new RectSelectTool(),
  {
    shortcut: 'M',
    hint: 'Rectangle Select (M). Shift: add, Alt: subtract, Ctrl+Shift: intersect'
  }
)

// Register the tool
ToolRegistry.register(RectSelectDefinition)
